                                ...
                                <div :class="cx('header')" v-bind="ptm('header')">
                                    ...
                                    <slot name="prevbutton" :actionCallback="(event) => onPrevButtonClick(event)" :keydownCallback="(event) => onContainerButtonKeydown(event)">
                                        <Button
                                            ...
                                            :disabled="disabled"
                                            :aria-label="currentView === 'year' ? $bumblevue.config.locale.prevDecade : currentView === 'month' ? $bumblevue.config.locale.prevYear : $bumblevue.config.locale.prevMonth"
                                            :unstyled="unstyled"
                                    ...
                                    <div :class="cx('title')" v-bind="ptm('title')">
                                        <template v-if="$bumblevue.config.locale.showMonthAfterYear">
                                            <button
                                    ...
                                    <slot name="nextbutton" :actionCallback="(event) => onNextButtonClick(event)" :keydownCallback="(event) => onContainerButtonKeydown(event)">
                                        <Button
                                            ...
                                            :disabled="disabled"
                                            :aria-label="currentView === 'year' ? $bumblevue.config.locale.nextDecade : currentView === 'month' ? $bumblevue.config.locale.nextYear : $bumblevue.config.locale.nextMonth"
                                            :unstyled="unstyled"
                    ...
                    <div v-if="(showTime || timeOnly) && currentView === 'date'" :class="cx('timePicker')" :data-p="timePickerDataP" v-bind="ptm('timePicker')">
                        <div :class="cx('hourPicker')" v-bind="ptm('hourPicker')" data-pc-group-section="timepickerContainer">
                            <slot name="hourincrementbutton" :callbacks="hourIncrementCallbacks">
                                <Button
                                    ...
                                    :aria-label="$bumblevue.config.locale.nextHour"
                                    :unstyled="unstyled"
                            ...
                            <slot name="hourdecrementbutton" :callbacks="hourDecrementCallbacks">
                                <Button
                                    ...
                                    :aria-label="$bumblevue.config.locale.prevHour"
                                    :unstyled="unstyled"
                                    @mousedown="onTimePickerElementMouseDown($event, 0, -1)"
                                    @mouseup="onTimePickerElementMouseUp($event)"
                                    @keydown="onContainerButtonKeydown"
                        ...
                        <div :class="cx('minutePicker')" v-bind="ptm('minutePicker')" data-pc-group-section="timepickerContainer">
                            <slot name="minuteincrementbutton" :callbacks="minuteIncrementCallbacks">
                                <Button
                                    :class="cx('pcIncrementButton')"
                                    :aria-label="$bumblevue.config.locale.nextMinute"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                            ...
                            <slot name="minutedecrementbutton" :callbacks="minuteDecrementCallbacks">
                                <Button
                                    :class="cx('pcDecrementButton')"
                                    :aria-label="$bumblevue.config.locale.prevMinute"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                                    @mousedown="onTimePickerElementMouseDown($event, 1, -1)"
                                    @mouseup="onTimePickerElementMouseUp($event)"
                        ...
                        <div v-if="showSeconds" :class="cx('secondPicker')" v-bind="ptm('secondPicker')" data-pc-group-section="timepickerContainer">
                            <slot name="secondincrementbutton" :callbacks="secondIncrementCallbacks">
                                <Button
                                    :class="cx('pcIncrementButton')"
                                    :aria-label="$bumblevue.config.locale.nextSecond"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                            ...
                            <slot name="seconddecrementbutton" :callbacks="secondDecrementCallbacks">
                                <Button
                                    :class="cx('pcDecrementButton')"
                                    :aria-label="$bumblevue.config.locale.prevSecond"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                                    @mousedown="onTimePickerElementMouseDown($event, 2, -1)"
                                    @mouseup="onTimePickerElementMouseUp($event)"
                        ...
                        <div v-if="hourFormat == '12'" :class="cx('ampmPicker')" v-bind="ptm('ampmPicker')">
                            <slot name="ampmincrementbutton" :toggleCallback="(event) => toggleAMPM(event)" :keydownCallback="(event) => onContainerButtonKeydown(event)">
                                <Button
                                    :class="cx('pcIncrementButton')"
                                    :aria-label="$bumblevue.config.locale.am"
                                    :disabled="disabled"
                                    :unstyled="unstyled"
                            ...
                            <span v-bind="ptm('ampm')" data-pc-group-section="timepickerlabel">{{ pm ? $bumblevue.config.locale.pm : $bumblevue.config.locale.am }}</span>
                            <slot name="ampmdecrementbutton" :toggleCallback="(event) => toggleAMPM(event)" :keydownCallback="(event) => onContainerButtonKeydown(event)">
                                <Button
                                    :class="cx('pcDecrementButton')"
                                    :aria-label="$bumblevue.config.locale.pm"
                                    :disabled="disabled"
                                    @click="toggleAMPM($event)"
                                    @keydown="onContainerButtonKeydown"
                                    v-bind="timepickerButtonProps"
...
<script>
import { cn } from '@cjdevstudios/bumbleuix-utils';
import { absolutePosition, addStyle, find, findSingle, getAttribute, getFocusableElements, getIndex, getOuterWidth, isTouchDevice, relativePosition, setAttribute } from '@cjdevstudios/bumbleuix-utils/dom';
import { isDate, isEmpty, isNotEmpty, localeComparator } from '@cjdevstudios/bumbleuix-utils/object';
import { ZIndex } from '@cjdevstudios/bumbleuix-utils/zindex';
import { ConnectedOverlayScrollHandler } from '@cjdevstudios/bumblevue-core/utils';
import CalendarIcon from '@cjdevstudios/bumblevue-icons/calendar';
import ChevronDownIcon from '@cjdevstudios/bumblevue-icons/chevrondown';
import ChevronLeftIcon from '@cjdevstudios/bumblevue-icons/chevronleft';
import ChevronRightIcon from '@cjdevstudios/bumblevue-icons/chevronright';
import ChevronUpIcon from '@cjdevstudios/bumblevue-icons/chevronup';
import TimesIcon from '@cjdevstudios/bumblevue-icons/times';
import Button from '@cjdevstudios/bumblevue/button';
import InputText from '@cjdevstudios/bumblevue/inputtext';
import OverlayEventBus from '@cjdevstudios/bumblevue/overlayeventbus';
...
export default {
    ...
    methods: {
        ...
        onOverlayEnter(el) {
            ...
            if (this.autoZIndex) {
                ZIndex.set('overlay', el, this.baseZIndex || this.$bumblevue.config.zIndex.overlay);
            }
                    ...
                    if (literal) {
                        ...
                        switch (format.charAt(iFormat)) {
                            case 'd':
                                ...
                                break;
                            case 'D':
                                output += formatName('D', date.getDay(), this.$bumblevue.config.locale.dayNamesShort, this.$bumblevue.config.locale.dayNames);
                                break;
                            case 'o':
                            ...
                            case 'm':
                                ...
                                break;
                            case 'M':
                                output += formatName('M', date.getMonth(), this.$bumblevue.config.locale.monthNamesShort, this.$bumblevue.config.locale.monthNames);
                                break;
                            case 'y':
        ...
        formatTime(date) {
            ...
            if (this.hourFormat === '12') {
                output += date.getHours() > 11 ? ` ${this.$bumblevue.config.locale.pm}` : ` ${this.$bumblevue.config.locale.am}`;
            }
        ...
        parseDateTime(text) {
            let date;
            const amLabel = this.$bumblevue.config.locale.am;
            const pmLabel = this.$bumblevue.config.locale.pm;
        ...
        populateTime(value, timeString, ampm) {
            ...
            this.pm = ampm.toLowerCase() === this.$bumblevue.config.locale.pm.toLowerCase() || ampm.toLowerCase() === 'pm';
                ...
                if (literal) {
                    ...
                    switch (format.charAt(iFormat)) {
                        case 'd':
                            ...
                            break;
                        case 'D':
                            getName('D', this.$bumblevue.config.locale.dayNamesShort, this.$bumblevue.config.locale.dayNames);
                            break;
                        case 'o':
                            doy = getNumber('o');
                            break;
                        case 'm':
                            ...
                            break;
                        case 'M':
                            month = getName('M', this.$bumblevue.config.locale.monthNamesShort, this.$bumblevue.config.locale.monthNames);
                            break;
                        case 'y':
                            year = getNumber('y');
                            break;
        ...
        createResponsiveStyle() {
            if (this.numberOfMonths > 1 && this.responsiveOptions && !this.isUnstyled) {
                if (!this.responsiveStyleElement) {
                    ...
                    setAttribute(this.responsiveStyleElement, 'nonce', this.$bumblevue?.config?.csp?.nonce);
    ...
    computed: {
        ...
        weekDays() {
            let weekDays = [];
            let dayIndex = this.$bumblevue.config.locale.firstDayOfWeek;

            for (let i = 0; i < 7; i++) {
                weekDays.push(this.$bumblevue.config.locale.dayNamesMin[dayIndex]);
        ...
        sundayIndex() {
            return this.$bumblevue.config.locale.firstDayOfWeek > 0 ? 7 - this.$bumblevue.config.locale.firstDayOfWeek : 0;
        },
        datePattern() {
            return this.dateFormat || this.$bumblevue.config.locale.dateFormat;
        },
        monthPickerValues() {
            ...
            for (let i = 0; i <= 11; i++) {
                monthPickerValues.push({
                    value: this.$bumblevue.config.locale.monthNamesShort[i],
        ...
        todayLabel() {
            return this.$bumblevue.config.locale.today;
        },
        clearLabel() {
            return this.$bumblevue.config.locale.clear;
        },
        weekHeaderLabel() {
            return this.$bumblevue.config.locale.weekHeader;
        },
        monthNames() {
            return this.$bumblevue.config.locale.monthNames;
        },
