    ...
    <div :ref="containerRef" :id="$id" :class="cx('root')" v-bind="ptmi('root')">
        ...
        <slot :id="$id" :name="$slots.button ? 'button' : 'menubutton'" :class="cx('button')" :toggleCallback="(event) => menuButtonClick(event)">
            <a
                ...
                :aria-controls="$id"
                :aria-label="$bumblevue.config.locale.aria?.navigation"
...
<script>
import { findSingle, focus, isTouchDevice } from '@cjdevstudios/bumbleuix-utils/dom';
    ...
    methods: {
        ...
        toggle(event) {
            if (this.mobileActive) {
                ...
                ZIndex.set('menu', this.menubar, this.$bumblevue.config.zIndex.menu);
                setTimeout(() => {
                    this.show();
                }, 1);
            }
